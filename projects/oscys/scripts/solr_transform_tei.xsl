<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
	...
	<xsl:template name="tei_document" exclude-result-prefixes="#all"></xsl:template>
	
	<!-- ================================================== -->
	<!--                   CASE DOC JOIN                    -->
	<!-- Info to go in a caseid record from a case document -->
	<!-- ================================================== -->
	...
	<xsl:template name="people"  exclude-result-prefixes="#all">
		...
		In order to match up the people in the documents with more than one case, they have been separated 
	...
	<xsl:template name="JSON_Formatter">
		...
		<xsl:if test="$json_date != ''">
			...
			<xsl:call-template name="escape-string">
				<xsl:with-param name="s" select="$json_date"/>
			</xsl:call-template>
			<xsl:text>"</xsl:text>
		...
		<xsl:if test="$json_id != ''">
			...
			<xsl:call-template name="escape-string">
				<xsl:with-param name="s" select="$json_id"/>
			</xsl:call-template>
			<xsl:text>"</xsl:text>
		</xsl:if>
	...
	<!-- 
		Main template for escaping strings; used by above template and for object
		-properties Responsibilities: placed quotes around string, and chain up 
		to next filter, escape-bs-string 
	-->
	...
	<!-- 
		...
		Can't escape backslash or double quote here, because they don't replace 
		characters (&#x0; becomes \t), but they prefix characters (\ becomes 
		\\). Besides, backslash should be seperate anyway, because it should 
		be processed first. This function can't do that. 
	-->
