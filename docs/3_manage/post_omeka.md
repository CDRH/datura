...
The standard definitions of Omeka fields are in the `field_definitions.py` file in `lib/datura/python/field_definitions.py`. The Omeka API fields defined here must match the Omeka resource template you are using, including the data type. Each field is specified by a corresponding method, all of which are called in `api_fields.py` to compile the Omeka S JSON. (For `update_item_value()`, the first item is the json hash, the second argument corresponds to the field in the resource template, and the third to the method in `field_definitions.py` Optionally, you can pass in the argument `datatype="[datatype]"` as the fourth argument.) If you do not specify it, it will be set to "literal". 
